...
def live_camera_inference():
    ...
    # JS: Space/S → click the webcam's built-in capture button
    gr.HTML("""
    <script>
    (function () {
        let _busy = false;
        ...
        function isBusy() {
            const bar = document.querySelector('.cam-status-bar');
            return bar ? bar.classList.contains('busy') : false;
        }

        function clickCaptureBtn() {
            // Gradio's webcam snapshot button — it's a <button> inside the webcam component
            // It appears as an SVG camera-icon button when in snapshot mode
            const btns = document.querySelectorAll('button');
            for (const btn of btns) {
                // The capture button contains a camera SVG or has aria-label with camera/capture
                const label = (btn.getAttribute('aria-label') || '').toLowerCase();
                const title = (btn.getAttribute('title') || '').toLowerCase();
                // Look for the capture/snapshot button by its icon or label
                if (label.includes('capture') || label.includes('photo') ||
                    title.includes('capture') || title.includes('photo') ||
                    btn.querySelector('svg[data-testid="camera"]') ||
                    btn.querySelector('svg') && btn.closest('.image-container, [data-testid="image"]')) {
                    btn.click();
                    return true;
                }
            }
        ...
        document.addEventListener('keydown', function (e) {
            if (_busy || isBusy()) return;
            if (e.code !== 'Space' && e.key !== 's' && e.key !== 'S') return;
            e.preventDefault();
            _busy = true;
            clickCaptureBtn();
        });
        ...
        new MutationObserver(() => {
            if (!isBusy()) _busy = false;
        }).observe(document.body, { subtree: true, childList: true, attributes: true });
    })();
    </script>
    """)

    def on_capture(frame, busy, selected_prompt):
        ...
        try:
            for partial in run_inference(frame, selected_prompt):
                if partial:
                    if not final_text:
...
def live_camera_voice_infer():
    ...
    async def process_voice_and_predict(image, audio_path):
        ...
        last_text = ""
        for text_out in run_inference(image, final_prompt):
            if text_out:
                last_text = text_out
...
def live_camera_continous_inference():
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            # FIX: gr.Label ko badal kar gr.JSON kiya taaki structured string metrics par validation crash na ho
    ...
    def on_frame(frame, last_run_time, is_streaming, selected_prompt):
        now = time.time()
        ...
        try:
            ...
            for partial in inference_stream:
                if partial:
